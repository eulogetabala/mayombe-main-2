...
const SharedCartScreen = () => {
  ...
  const handleLoadSharedCart = async () => {
    ...
    try {
      ...
      if (sharedCartJson) {
        ...
        const cartItems = cartData.items || cartData; // Support de l'ancienne structure
        ...
        if (cartItems && cartItems.length > 0) {
          ...
          const transformedCart = cartItems.map(item => ({
            ...
            imageUrl: item.imageUrl || (item.cover 
              ? `https://www.mayombe-app.com/uploads_admin/${item.cover}`
              : null),
            image: item.image || (item.cover 
              ? { uri: `https://www.mayombe-app.com/uploads_admin/${item.cover}` }
  ...
  const handlePayNow = () => {
    ...
    if (setCartItems) {
      ...
      AsyncStorage.setItem('cart', JSON.stringify(sharedCart));
    }
    Alert.alert(
      'Paiement',
  ...
  const renderCartItem = ({ item }) => (
    <View style={styles.cartItem}>
      <View style={styles.itemImage}>
        {item.image ? (
          <Image source={item.image} style={styles.itemImageSource} />
        ) : (
