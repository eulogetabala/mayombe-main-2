  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (status !== 'granted') {
          console.log('Permission localisation refusée - RestaurantListScreen - Utilisation défaut');
        ...
        if (location) {
          ...
          console.log('Position technique impossible - RestaurantListScreen - Utilisation défaut');
          setUserLocation({ latitude: -4.2634, longitude: 15.2429 });
        }
      } catch (error) {
        console.error('Erreur localisation RestaurantListScreen:', error);
        setUserLocation({ latitude: -4.2634, longitude: 15.2429 });
      }
    })();
  }, [city]);
      ...
      if (Array.isArray(data)) {
        ...
        const enrichedRestaurants = mappedRestaurants.map(restaurant => {
          ...
          const coverImage = resolveImageUrl(
            images.cover,
            restaurant.cover || data.find(r => r.id === restaurant.id)?.cover,
