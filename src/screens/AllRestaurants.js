  ...
  const fetchRestaurants = async () => {
    try {
      ...
      if (response.ok && Array.isArray(data)) {
        // Si un cityId est fourni, on fait confiance à l'API et on filtre seulement par statut actif
        // Sinon, on filtre par statut actif ET par nom de ville (Brazzaville ou Pointe-Noire)
        const activeRestaurants = data.filter(restaurant => {
          const isActive = restaurant.statut === "actif";
          
          // Si un cityId est fourni, on ne filtre pas par nom de ville
          if (cityId) {
            return isActive;
          }
          
          // Sinon, on filtre par nom de ville
      ...
      setRestaurants(prevRestaurants => {
        return prevRestaurants.map(restaurant => {
          ...
          if (allStatuses[restaurantIdStr]) {
            ...
            if (status.cover) {
              updated.image = resolveImageUrl(status.cover, updated.cover, require("../../assets/images/2.jpg"));
            }
            if (status.logo) {
