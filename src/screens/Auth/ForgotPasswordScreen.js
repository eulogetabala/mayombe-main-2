...
const ForgotPasswordScreen = ({ navigation }) => {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      const isSuccess = response.success || response.status === 'success' || response.message?.includes('succès') || response.message?.includes('envoyé');
      
      // Si le code est envoyé avec succès (même si la réponse dit "erreur" mais contient "envoyé")
      if (isSuccess || (response.message && (response.message.includes('envoyé') || response.message.includes('succès')))) {
        Alert.alert(
          "Code envoyé",
          ...
          [
            {
              text: "OK",
              onPress: () => navigation.navigate("ForgotPasswordOtp", { phone: fullPhone })
            }
          ]
        );
      } else {
