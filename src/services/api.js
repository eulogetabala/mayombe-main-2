...
export const api = {
  ...
  resetPassword: async (otp, newPassword) => {
    try {
      ...
      // Essayer différents formats selon la spécification API
      const formats = [
        { otp, 'new password': newPassword },
        { otp, new_password: newPassword },
      ...
      for (let i = 0; i < formats.length; i++) {
        try {
          console.log(`Tentative format ${i + 1}:`, formats[i]);
          ...
          const data = await response.json();
          console.log(`Réponse API reset password (format ${i + 1}):`, data);
          
          // Si succès, retourner le résultat
          if (data.message?.includes('succès') || data.message?.includes('changé')) {
            console.log(`Format ${i + 1} fonctionne !`);
            return data;
          }
        } catch (error) {
          console.log(`Format ${i + 1} échoué:`, error.message);
        }
      }
      
      // Si aucun format ne fonctionne, retourner le dernier résultat
      return { error: 'Aucun format ne fonctionne' };
    } catch (error) {
